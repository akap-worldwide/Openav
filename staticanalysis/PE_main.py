...
features = pickle.loads(open(os.path.join('staticanalysis/Classifier/features.pkl'),'rb').read())
clf = joblib.load('staticanalysis/Classifier/classifier.pkl')


def isMalware(file):
   
    data = extract_infos(file) #getting features....


    pe_features = list(map(lambda x:data[x], features))
    res= clf.predict_proba([pe_features])[0][0]*100
...
def MLscanfiles(files):
    malwares = []
    for x in files:
        t = magic.from_file(x,mime=True)
